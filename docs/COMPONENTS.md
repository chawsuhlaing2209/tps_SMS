...
| **2. Scope the *same* list, 2–5 fixed options** — a filter/mode toggle, all options always visible | `SegmentedControl` | `components/pds` | For archive scope specifically, use the specialization **`ArchiveVisibilityFilter`** (`components/shared/archive-visibility-filter`) — never rebuild Active/Archived/All by hand. **Placement: always right-aligned** — PageHeader `actions`, `PdsSearchFiltersRow`'s `statusControl` slot, or a `justify-content: flex-end` toolbar. Other examples: EN/MY switcher, calendar modes. |
